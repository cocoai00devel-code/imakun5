...
//        // ★1: 周波数データを取得
//        analyser.getByteFrequencyData(dataArray); // 音声データを取得
       
//        // ★2: step を計算
//        const step = Math.floor(dataArray.length / barCount);

//        bars.forEach((bar, i) => {
//         const volume = dataArray[i * step] / 255; // 0〜1に正規化
//         let targetHeight = volume * 180 + 20;     // 音量に合わせて高さを計算
...
// }
   /* ========================================================
    ...
    //         // 【合体ロジック】
    //         // 声の大きさ（volume * 150）に、ランダムな揺らぎ（20〜40程度）をミックス
    //         let targetHeight = (volume * 150) + (Math.random() * 20 + 20);
...
//     if(isRecording) {
//             // 音声認識中は常に明るく（不透明）
//         barColor = barColor.replace(/0\.\d+\)/, '1)');
...
//         const g = startRgb[1] + (endRgb[1] - startRgb[1]) * segmentProgress;
//         const b = startRgb[2] + (endRgb[2] - startRgb[2]) * segmentProgress;
//         barColor = rgbToHex(r, g, b) + 'b3' ; // 完全不透明
//     } else if (currentWaveColor === 'rainbow') {
//         rainbowHue = (typeof rainbowHue !== 'undefined' ? rainbowHue + 3 : 0) % 360;
//         barColor = `hsla(${rainbowHue}, 100%, 70%, 0.9)`;
//     }
...
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    ...
    if (isRecording) {
        // 音声認識中は毎フレーム明るく＋微妙に変化
        if (barColor.startsWith('rgba')) {
            barColor = barColor.replace(/0\.\d+\)/, '1)'); // 不透明
            const match = barColor.match(/rgba\((\d+),\s*(\d+),\s*(\d+),\s*1\)/);
            if (match) {
                let r = Math.min(255, parseInt(match[1]) + Math.random() * 50);
                let g = Math.min(255, parseInt(match[2]) + Math.random() * 50);
                let b = Math.min(255, parseInt(match[3]) + Math.random() * 50);
                barColor = `rgba(${r|0}, ${g|0}, ${b|0}, 1)`;
            }
        } else if (barColor.startsWith('#')) {
            let [r, g, b] = hexToRgb(barColor);
            r = Math.min(255, r + Math.random() * 50);
            g = Math.min(255, g + Math.random() * 50);
            b = Math.min(255, b + Math.random() * 50);
            barColor = `rgba(${r|0}, ${g|0}, ${b|0}, 1)`;
        } else if (barColor.startsWith('hsl')) {
            const match = barColor.match(/hsl\((\d+),\s*(\d+)%,\s*(\d+)%/);
            if (match) {
                let h = (parseInt(match[1]) + Math.random() * 30) % 360;
                let s = match[2];
                let l = match[3];
                barColor = `hsl(${h}, ${s}%, ${l}%)`;
            }
    ...
    // ===== TTS応答中 =====
        ...
        bars.forEach((bar, i) => {
            const waveOffset = Math.sin(Date.now() * 0.005 + i * 0.05) * 100;
            let targetHeight = 10 + Math.abs(waveOffset) + (Math.random() * 80 + 20);
            bar.height += (targetHeight - bar.height) * 0.15;
        });
    ...
    // ===== 待機中 =====
        ...
        bars.forEach((bar, i) => {
            let idleWave = Math.sin(Date.now() * 0.002 + i * 0.2) * 5;
            let targetHeight = 4 + idleWave + Math.random() * 12;
            bar.height += (targetHeight - bar.height) * 0.15;
        });
    }
...
//     // 【補正】声の周波数（前半50%）に絞って計算
//     // const activeDataLength = dataArray.length * 0.5;
//     // const step = Math.floor(activeDataLength / bars.length); 
...
//     //         }
//     //     }
//     //     const volume = (sum / sampleRange) / 255; 

//     //     // 倍率130、最小揺らぎ8で「ちょうど良い」動きに設定
//     //     let targetHeight = (volume * 130) + (Math.random() * 5 + 8); 
...
//     // else if (isTtsSpeaking) {
//     //     bars.forEach((bar, i) => {
//     //         // サイン波に基づいた滑らかなアニメーション
//     //         const waveOffset = Math.sin(Date.now() * 0.005 + i * 0.05) * 100;
//     //         let targetHeight = 10 + Math.abs(waveOffset);
            
//     //         bar.height += (targetHeight - bar.height) * 0.15;
//     //     });
//     //      bars.forEach((bar, i) => {
//     //     // --- 修正箇所：ただの 10 ではなく、滑らかに動かす ---
//     //     // 10から20の範囲でターゲットを決める
//     //     let targetHeight = 10 + Math.random() * 10;
//     //     // 現在の高さからターゲットへ 15% ずつ近づける（これでヌルヌル動く）
//     //     bar.height += (targetHeight - bar.height) * 0.15;
//     // });
//     // }

//     /* ========================================================
//    ② TTS応答中（AIが喋っている時）: 合体アニメーション
// ======================================================== */
//     } else if (isTtsSpeaking) {
//         bars.forEach((bar, i) => {
//             // 1. 大きなうねり（サイン波）を計算
...
//             // 2. 「ベースのうねり」に「10〜20のランダムな震え」を合体
//             // Math.abs(waveOffset) で大きな動きを作り、そこにランダムを足します
//             let targetHeight = 10 + Math.abs(waveOffset) + (Math.random() * 80 + 20);

//             // 3. 0.15の係数でヌルヌルと補間
//             // これにより、大きな波の中でも細かく滑らかに震える動きになります
//             bar.height += (targetHeight - bar.height) * 0.15;
//         });
//     }
//     /* ========================================================
//        ③ 待機中（うねるような控えめな動き）の計算
//     ======================================================== */
//     else {
   
//     // AI応答中の擬似的な動的波形
//     // bars.forEach(bar => {
//     //     // 20から100（80+20）の範囲でランダムに動く
...
//     bars.forEach((bar, i) => {
//         // 時間経過（Date.now）とバーの位置（i）を使って、うねるような動きを作る
//         let idleWave = Math.sin(Date.now() * 0.002 + i * 0.2) * 5;
//         let targetHeight = 4 + idleWave + Math.random() * 12;
//         // let targetHeight = 10 + Math.abs(idleWave) + (Math.random() * 10);
//         bar.height += (targetHeight - bar.height) * 0.15;
    // });
